...
class Executor(object):
    ...
    def test(self, mark=''):
        ...
        folder = os.path.join(self.conf.folder, 'test_results_%s' % self.conf.test_dataset + postfix)
        ...
        folder = os.path.join(self.conf.folder, 'training_results_%s'% self.conf.test_dataset + postfix)
        if not os.path.exists(folder):
            os.makedirs(folder)
        ...
        folder = os.path.join(self.conf.folder, 'validation_results_%s' % self.conf.test_dataset + postfix)
        if not os.path.exists(folder):
            os.makedirs(folder)
    ...
    def test_modality(self, folder, modality, group, save_figs=True):
        ...
        for vol_i in test_data.volumes():
            ...
            patho_pred_list = []
            for pred in patho_pred:
                patho_pred_list.append(pred[:,:,:,0:1])
            patho_pred = np.concatenate(patho_pred_list, axis=-1)
            ...
            im_dice_anato[vol_i], sep_dice_anato \
                = dice(vol_anato_mask, pred[0], model_type=model_type)
            im_false_negative_anato[vol_i], sep_false_negative_anato \
                = calculate_false_negative(vol_anato_mask, pred[0], model_type=model_type)

            im_dice_patho[vol_i], sep_dice_patho \
                = dice(vol_patho_mask, pred[1], model_type=model_type)
            im_false_negative_patho[vol_i], sep_false_negative_patho \
                = calculate_false_negative(vol_patho_mask, pred[1], model_type=model_type)
...
def data_generator_multiple_outputs(datagen, batch_size, inp, outs):
    ...
    #gen_iterator = datagen.flow(x=x,y=y,batch_size=batch_size, seed=1)
    ...
    while True:
        ...
        # batch_x, batch_y_raw = next(gen_iterator)
        batch_y = []
        for ii in range(len(outs)):
