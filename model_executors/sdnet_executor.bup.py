...
class SDNetExecutor(Executor):
    ...
    def _load_discriminator_masks(self):
        ...
        # for vol in volumes:
        #     images = temp.get_images(vol)
        #     anato_masks = temp.get_anato_masks(vol)
        #     patho_masks = temp.get_patho_masks(vol)
        #     scanner = temp.get_scanner(vol)
        #     slice = temp.get_slice(vol)
        #     num = int(self.conf.pctg_per_volume * images.shape[0])
        ...
        #
        # temp.images = np.concatenate(new_images, axis=0)
        # temp.anato_masks = np.concatenate(new_anato_masks, axis=0)
        # temp.patho_masks = np.concatenate(new_patho_masks, axis=0)
        # temp.scanner = np.concatenate(new_scanner, axis=0)
        # temp.slice = np.concatenate(new_slice, axis=0)
        # temp.index = np.concatenate(new_index, axis=0)
        #
        # anato_masks = temp.anato_masks
        # patho_masks = temp.patho_masks
        # images = temp.images
    ...
    def _init_discriminator_image_generator(self):
        ...
        # new_images, new_anato_masks = [], []
        # for vol in volumes:
        #     images = data.get_images(vol)
        #     anato_masks = data.get_anato_masks(vol)
        #     num = int(self.conf.pctg_per_volume * images.shape[0])
        #     idx = np.random.choice(images.shape[0], size=num, replace=False)
        #     images = np.array([images[i] for i in idx])
        #     anato_masks = np.array([anato_masks[i] for i in idx])
        ...
        #     new_anato_masks.append(anato_masks)
        #
        # data.images = np.concatenate(new_images, axis=0)
        # data.anato_masks = np.concatenate(new_anato_masks, axis=0)
        # anato_masks = data.anato_masks
        # images = data.images
        #
        # del data
        ...
        # anato_masks = data.anato_masks.copy()
    ...
    def train(self):
        ...
        for self.epoch in range(self.conf.epochs):
            ...
            for self.batch in range(self.conf.batches):
                ...
                # if self.conf.batches > check_batch_iters and total_iters % check_batch_iters == 0:
                #     folder = os.path.join(os.path.join(self.conf.folder, 'test_during_train'),
                #                           'test_results_%s_epoch%d_iter%d'
                #                           % (self.conf.test_dataset, self.epoch, self.batch))
                #     if not os.path.exists(folder):
                #         os.makedirs(folder)
                #     test_performance = self.test_modality(folder, self.conf.modality, 'test')
    ...
    def train_batch_generators_unlabeld_pathology(self, epoch_loss, lr_callback):
        if self.gen_unlabelled is not None:
            ...
            h_pppa_reconst = self.model.G_trainer_up_pppa_reconst.\
                fit([x], x, epochs=1, verbose=0, callbacks=[lr_callback])
    ...
    def train_batch_generators_labeled_pathology(self, epoch_loss, lr_callback):
        ...
        if self.gen_labelled is not None:
            ...
            #        + h_pppa_reconst.history['loss'][-1] + h_ppra_reconst.history['loss'][-1] \
            #        + h_rp.history['loss'][-1] + h_rp_z.history['loss'][-1]