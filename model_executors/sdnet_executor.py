...
class SDNetExecutor(Executor):
    ...
    def __init__(self, conf, model):
        ...
        self.tensorboard_dir = os.path.join(self.conf.folder, 'TensorboardLogs')
        if not os.path.exists(self.tensorboard_dir):
            os.makedirs(self.tensorboard_dir)
        # self.tensorboard = TensorBoard(self.tensorboard_dir)
    ...
    def _init_labelled_data_generator(self):
        ...
        return itertools.zip_longest(img_gen, anato_msk_gen, patho_msk_gen)
    ...
    def get_loss_names(self):
        ...
        return ['Loss_Total_Generator_LP',
                ...
                'Adv_Reconstruction_Generator_PH',

                'Clsf_Reconstruction_Generator_RP',
                'Clsf_Reconstruction_Generator_PPPA',
                'Clsf_Reconstruction_Generator_PPRA',
                'Clsf_Reconstruction_Generator_PH',


                'Adv_Reconstruction_Discriminator_RP',
                'Adv_Reconstruction_Discriminator_PPPA',
                'Adv_Reconstruction_Discriminator_PPRA',
                'Adv_Reconstruction_Discriminator_PH',

                'Clsf_Reconstruction_Discriminator_RP',
                'Clsf_Reconstruction_Discriminator_PPPA',
                'Clsf_Reconstruction_Discriminator_PPRA',
                'Clsf_Reconstruction_Discrirminator_PH',
                ...
                'Reconstruct_S_RP',
                'Reconstruct_S_PPPA', 'Reconstruct_S_PPRA',
                'Reconstruct_S_PH',

                'Reconstruct_P_RP',
                'Reconstruct_P_PPPA', 'Reconstruct_P_PPRA',
                'Reconstruct_P_PH',
    ...
    def train(self):
        ...
        for self.epoch in range(self.conf.epochs):
            ...
            # val_dice = self.validate(epoch_loss, model_type='sdnet')
            ...
            if 'sdnet' in self.conf.executor:
                model_type = 'sdnet'
            elif 'base' in self.conf.executor:
                model_type = 'unet'
            val_dice = self.validate(epoch_loss, model_type=model_type)
    ...
    def validate(self, epoch_loss, model_type):
        ...
        pathology_pred_mask_list = []
        for pred in pathology_pred_mask:
            curt_pred = pred[:,:,:,0:1]
            pathology_pred_mask_list.append(curt_pred)
        pathology_pred_mask = np.concatenate(pathology_pred_mask_list, axis=-1)
        ...
        return costs.dice(valid_data.patho_masks,pathology_pred_mask,model_type=model_type)[0]
    ...
    def discriminator_train_with_labeled_pathology(self, epoch_loss, lr_callback):
        ...
        m_patho_pseudo_health = np.zeros(shape=m_patho.shape, dtype=m_patho.dtype)

        fake_s = self.model.Enc_Anatomy.predict(x_masks)
        fake_m_real_pathology = self.model.Enc_Modality.predict([fake_s, m_patho, x_masks])[0:self.conf.input_shape[-1]]
        fake_rec_real_pathology_real_pathology_list = []
        for ii in range(len(fake_m_real_pathology)):
            fake_rec_real_pathology_real_pathology = self.model.Decoder.predict([fake_s, m_patho, fake_m_real_pathology[ii]])
            fake_rec_real_pathology_real_pathology_list.append(fake_rec_real_pathology_real_pathology)
        fake_rec_real_pathology_real_pathology = np.concatenate(fake_rec_real_pathology_real_pathology_list,axis=-1)


        # fake_m_pseudo_health = self.model.Enc_Modality.predict([fake_s, m_patho_pseudo_health, x_masks])[0]
        # fake_rec_real_pathology_real_pathology = self.model.Decoder.predict([fake_s, m_patho, fake_m_real_pathology])
        # fake_rec_pseudo_health = self.model.Decoder.predict([fake_s, m_patho, fake_m_pseudo_health])
        # fake_m = self.model.Segmentor.predict(fake_s)

        # Train Discriminator
        image_shape = (batch_size,) + self.model.D_Reconstruction.get_output_shape_at(0)[0][1:]
        actual_classification_label_batch = np.array(np.array(np.sum(m_patho, axis=(1, 2, 3)), dtype=bool),
                                                     dtype=np.uint8)
        one_hot_actual_target = np.eye(2)[actual_classification_label_batch]
        # one_hot_pseudo_health = np.concatenate([np.ones(shape=(x_masks.shape[0], 1), dtype=one_hot_actual_target.dtype),
        #                                         np.zeros(shape=(x_masks.shape[0], 1),
        #                                                  dtype=one_hot_actual_target.dtype)],
        #                                        axis=1)
            ...
            self.model.D_Reconstruction_trainer_lp_rp. \
                fit([x_masks, fake_rec_real_pathology_real_pathology],
                    [np.ones(image_shape),
                     ...
                     one_hot_actual_target,
                     one_hot_actual_target,
                     np.zeros(batch_size),
                     np.zeros(batch_size)],
                    epochs=1, verbose=0, callbacks= [lr_callback])

        # h_reconstruction_discriminator_ph = \
        #     self.model.D_Reconstruction_trainer_up_ph. \
        #         fit([x_masks, fake_rec_pseudo_health],
        #             [np.ones(image_shape),
        #              np.zeros(image_shape),
        #              one_hot_pseudo_health,
        #              one_hot_pseudo_health,
        ...
        # epoch_loss['Adv_Reconstruction_Discriminator_PH']. \
        #     append([(h_reconstruction_discriminator_ph.history[
        #                  'Adv_Reconstruction_ActualPathology_Real_PseudoHealth_loss'][-1] +
        #              h_reconstruction_discriminator_ph.history[
        #                  'Adv_Reconstruction_ActualPathology_Fake_PseudoHealth_loss'][-1])])
        epoch_loss['Clsf_Reconstruction_Discriminator_RP']. \
            append([(h_reconstruction_discriminator_rp.history[
                         'Clsf_Reconstruction_ActualPathology_Real_ActualPathology_loss'][-1] +
                     h_reconstruction_discriminator_rp.history[
                         'Clsf_Reconstruction_ActualPathology_Fake_ActualPathology_loss'][-1])])
        # epoch_loss['Clsf_Reconstruction_Discrirminator_PH']. \
        #     append([(h_reconstruction_discriminator_ph.history[
        #                  'Clsf_Reconstruction_ActualPathology_Real_PseudoHealth_loss'][-1] +
        #              h_reconstruction_discriminator_ph.history[
        #                  'Clsf_Reconstruction_ActualPathology_Fake_PseudoHealth_loss'][-1])])
    ...
    def discriminator_train_with_unlabeled_pathology(self, epoch_loss, lr_callback):
        ...
        fake_z_ppra = self.model.Enc_Modality.predict([fake_s,
                                                       np.concatenate([predicted_pathology_ppra[0][:,:,:,0:1],
                                                                       predicted_pathology_ppra[1][:,:,:,0:1]], axis=-1),
                                                       x_masks])[0:self.conf.input_shape[-1]]
        fake_rec_ppra_list = []
        for ii in range(self.conf.input_shape[-1]):
            fake_rec_ppra = self.model.Decoder.predict([fake_s,
                                                        np.concatenate([predicted_pathology_ppra[0][:,:,:,0:1],
                                                                        predicted_pathology_ppra[1][:,:,:,0:1]], axis=-1),
                                                        fake_z_ppra[ii]])
            fake_rec_ppra_list.append(fake_rec_ppra)
        fake_rec_ppra = np.concatenate(fake_rec_ppra_list,axis=-1)

        predicted_pathology_pppa = self.model.Enc_Pathology.predict(np.concatenate([x_masks, fake_m], axis=-1))
        fake_z_pppa = self.model.Enc_Modality.predict([fake_s,
                                                       np.concatenate([predicted_pathology_pppa[0][:,:,:,0:1],
                                                                       predicted_pathology_pppa[1][:,:,:,0:1]], axis=-1),
                                                       x_masks])[0:self.conf.input_shape[-1]]
        fake_rec_pppa_list = []
        for ii in range(self.conf.input_shape[-1]):
            fake_rec_pppa = self.model.Decoder.predict([fake_s,
                                                        np.concatenate([predicted_pathology_pppa[0][:,:,:,0:1],
                                                                        predicted_pathology_pppa[1][:,:,:,0:1]], axis=-1),
                                                        fake_z_pppa[ii]])
            fake_rec_pppa_list.append(fake_rec_pppa)
        fake_rec_pppa = np.concatenate(fake_rec_pppa_list, axis=-1)
        ...
        epoch_loss['Clsf_Reconstruction_Discriminator_PPPA']. \
            append([(h_reconstruction_discriminator_pppa_supervised.history[
                         'Clsf_Reconstruction_ActualPathology_Real_ActualPathology_loss'][-1] +
                     h_reconstruction_discriminator_pppa_supervised.history[
                         'Clsf_Reconstruction_ActualPathology_Fake_ActualPathology_loss'][-1])])
        epoch_loss['Clsf_Reconstruction_Discriminator_PPRA']. \
            append([(h_reconstruction_discriminator_ppra_supervised.history[
                         'Clsf_Reconstruction_ActualPathology_Real_ActualPathology_loss'][-1] +
                     h_reconstruction_discriminator_ppra_supervised.history[
                         'Clsf_Reconstruction_ActualPathology_Fake_ActualPathology_loss'][-1])])
    ...
    def train_batch_generators_unlabeld_pathology(self, epoch_loss, lr_callback):
        if self.gen_unlabelled is not None:
            x, anato_m, _ = next(self.gen_unlabelled)
            pseudo_health_m = np.zeros(shape=tuple(anato_m.shape[:-1]
                                                   + (self.conf.num_pathology_masks,)), dtype=anato_m.dtype)
            batch_size = x.shape[0]
            actual_classification_label_batch = np.array(np.array(np.sum(_, axis=(1, 2, 3)), dtype=bool), dtype=np.uint8)
            one_hot_actual_target = np.eye(2)[actual_classification_label_batch]
            ...
            h_pppa = self.model.G_trainer_up_pppa. \
                fit([x],
                    [np.zeros(batch_size) for _ in range(self.conf.input_shape[-1])]
            ...
            h_triplet = self.model.G_trainer_up_triplet. \
                fit([x, pseudo_health_m, anato_m],
                    [np.zeros(batch_size),
                     np.zeros(batch_size),
                     np.ones((batch_size,) + self.model.D_Reconstruction.output_shape[0][1:]),
                     np.ones((batch_size,) + self.model.D_Reconstruction.output_shape[1][1:])
                     ],
                    epochs=1, verbose=0, callbacks= [lr_callback])
            ...
            predicted_pathology_from_real_anatomy = self.model.Enc_Pathology.predict(np.concatenate([x, m_anato_with_background], axis=-1))
            sample_z2 = NormalDistribution().sample((batch_size, self.conf.num_z * self.conf.input_shape[-1]))
            sample_z3 = NormalDistribution().sample((batch_size, self.conf.num_z * self.conf.input_shape[-1]))
            # sample_z4 = NormalDistribution().sample((batch_size, self.conf.num_z))
            # rec2 = self.model.Decoder.predict([s, predicted_pathology_from_predicted_anatomy[:,:,:,:-1], sample_z2])
            # rec3 = self.model.Decoder.predict([s, predicted_pathology_from_real_anatomy[:,:,:,:-1], sample_z3])
            # rec4 = self.model.Decoder.predict([s, pseudo_health_m, sample_z4])
            #
            #
            h_pppa_z = self.model.z_reconstructor. \
                fit([s, np.concatenate([predicted_pathology_from_predicted_anatomy[0][:,:,:,0:1],
                                        predicted_pathology_from_predicted_anatomy[1][:,:,:,0:1]],
                                       axis=-1),
            ...
            # h_pppa_s = self.model.s_reconstructor. \
            #     fit([rec2], s, epochs=1, verbose=0,
            #         callbacks=[SingleWeights_Callback(self.conf.w_rec_s * self.conf.pred_pathology_weight_rate * self.conf.pred_anatomy_weight_rate + eps,
            #                                           model=self.model.s_reconstructor),lr_callback])
            # h_pppa_p = self.model.p_reconstructor. \
            #     fit([rec2], predicted_pathology_from_predicted_anatomy[:,:,:,:-1], epochs=1, verbose=0,
            #         callbacks=[SingleWeights_Callback(self.conf.w_rec_p * self.conf.pred_pathology_weight_rate * self.conf.pred_anatomy_weight_rate + eps,
            #                                           model=self.model.p_reconstructor),lr_callback])

            h_ppra_z = self.model.z_reconstructor. \
                fit([s, np.concatenate([predicted_pathology_from_real_anatomy[0][:, :, :, 0:1],
                                        predicted_pathology_from_real_anatomy[1][:, :, :, 0:1]],
                                       axis=-1),
            ...
            # h_ppra_s = self.model.s_reconstructor. \
            #     fit([rec3], s, epochs=1, verbose=0,
            #         callbacks=[SingleWeights_Callback(
            #             self.conf.w_rec_s * self.conf.pred_pathology_weight_rate * self.conf.real_anatomy_weight_rate + eps,
            #             model=self.model.s_reconstructor)])
            # h_ppra_p = self.model.p_reconstructor. \
            #     fit([rec3], predicted_pathology_from_real_anatomy[:,:,:,:-1], epochs=1, verbose=0,
            #         callbacks=[SingleWeights_Callback(
            #             self.conf.w_rec_p * self.conf.pred_pathology_weight_rate * self.conf.real_anatomy_weight_rate + eps,
            #             model=self.model.p_reconstructor),lr_callback])
            ...
            # epoch_loss['Reconstruct_P_PPPA']. \
            #     append([h_pppa_p.history['loss'][-1] / (self.conf.w_rec_p
            #                                             * self.conf.pred_pathology_weight_rate
            #                                             * self.conf.pred_anatomy_weight_rate + eps)])
            # epoch_loss['Reconstruct_P_PPRA']. \
            #     append([h_ppra_p.history['loss'][-1] / (self.conf.w_rec_p
            #                                             * self.conf.pred_pathology_weight_rate
            #                                             * self.conf.real_anatomy_weight_rate + eps)])
            #
            # epoch_loss['Reconstruct_S_PPPA']. \
            #     append([h_pppa_p.history['loss'][-1] / (self.conf.w_rec_s
            #                                             * self.conf.pred_pathology_weight_rate
            #                                             * self.conf.pred_anatomy_weight_rate + eps)])
            # epoch_loss['Reconstruct_S_PPRA']. \
            #     append([h_ppra_p.history['loss'][-1] / (self.conf.w_rec_s
            #                                             * self.conf.pred_pathology_weight_rate
            #                                             * self.conf.real_anatomy_weight_rate + eps)])

            # return h_pppa.history['loss'][-1] + h_pppa_reconst.history['loss'][-1]\
            #        + h_ppra.history['loss'][-1] + h_ppra_reconst.history['loss'][-1] \
            #        + h_anatomy.history['loss'][-1]  \
    ...
    def train_batch_generators_labeled_pathology(self, epoch_loss, lr_callback):
        ...
        if self.gen_labelled is not None:
            x, anato_m, patho_m = next(self.gen_labelled)
            ...
            h_pppa = self.model.G_trainer_lp_pppa. \
                fit([x, patho_m],
                    [patho_m[:,:,:,0:1],patho_m[:,:,:,1:2], patho_m[:,:,:,0:1],patho_m[:,:,:,1:2],
                     one_hot_actual_target],
                    epochs=1, verbose=0, callbacks= [lr_callback])
            ...
            epoch_loss['SegDice_Patho_PPPA'].append((h_pppa.history['Dice_Patho1_loss'][-1]
                                                     +h_pppa.history['Dice_Patho2_loss'][-1])/2)
            epoch_loss['SegCrossEntropy_Patho_PPPA'].append((h_pppa.history['CrossEntropy_Patho1_loss'][-1]
                                                             +h_pppa.history['CrossEntropy_Patho2_loss'][-1])/2)
            epoch_loss['SegDice_Patho_PPRA'].append((h_ppra.history['Dice_Patho1_loss'][-1]
                                                     +h_ppra.history['Dice_Patho2_loss'][-1])/2)
            epoch_loss['SegCrossEntropy_Patho_PPRA'].append((h_ppra.history['CrossEntropy_Patho1_loss'][-1]
                                                             +h_ppra.history['CrossEntropy_Patho2_loss'][-1])/2)
            ...
            #epoch_loss['Clsf_Reconstruction_Generator_RP'].append(h_rp.history['Generated_Classification_RP_loss'][-1])
            # epoch_loss['Clsf_Reconstruction_Generator_PPPA'].append(h_pppa.history['Generated_Classification_PPPA_loss'][-1])
            # epoch_loss['Clsf_Reconstruction_Generator_PPRA'].append(h_ppra.history['Generated_Classification_PPRA_loss'][-1])

            # # Train Z Regressor
            s = self.model.Enc_Anatomy.predict(x)
            sample_z1 = NormalDistribution().sample((batch_size, self.conf.num_z * self.conf.input_shape[-1]))
            # rec = self.model.Decoder.predict([s, patho_m, sample_z1])
            ...
            # h_rp_s = self.model.s_reconstructor.\
            #     fit([rec], s, epochs=1, verbose=0,
            #         callbacks = [SingleWeights_Callback(self.conf.w_rec_s*self.conf.real_pathology_weight_rate+eps,
            #                                             model=self.model.s_reconstructor)])
            # h_rp_p = self.model.p_reconstructor.\
            #     fit([rec], patho_m, epochs=1, verbose=0,
            #         callbacks = [SingleWeights_Callback(self.conf.w_rec_p*self.conf.real_pathology_weight_rate+eps,
            #                                             model=self.model.p_reconstructor)])
            #
            #
            epoch_loss['Reconstruct_Z_RP'].append([h_rp_z.history['loss'][-1] /
                                                   (self.conf.w_rec_Z * self.conf.real_pathology_weight_rate + eps)])
            # epoch_loss['Reconstruct_S_RP'].append([h_rp_s.history['loss'][-1] /
            #                                        (self.conf.w_rec_s * self.conf.real_pathology_weight_rate + eps)])
            # epoch_loss['Reconstruct_P_RP'].append([h_rp_p.history['loss'][-1] /
            #                                        (self.conf.w_rec_p * self.conf.real_pathology_weight_rate + eps)])

            # return h_rp.history['loss'][-1] \
            #        + h_pppa.history['loss'][-1] + h_pppa_reconst.history['loss'][-1]\
            #        + h_ppra.history['loss'][-1] + h_ppra_reconst.history['loss'][-1] \
            #        + h_triplet.history['loss'][-1]
