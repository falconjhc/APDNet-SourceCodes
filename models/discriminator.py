...
class Discriminator(BaseNet):
    ...
    def build(self):
        ...
        if not self.conf.auxiliary_classifier and self.conf.triplet_output:
            self.model = Model(d_input, l_discrimination, name=name)
        elif self.conf.auxiliary_classifier and not self.conf.triplet_output:
            l_classification = Flatten()(l_before_output)
            # l_classification = Dense(2, activation='softmax')(l_classification)
            l_classification = Dense(2)(l_classification)
            l_classification = Lambda(lambda x: tf.nn.softmax(x))(l_classification)
            self.model = Model(d_input, [l_discrimination, l_classification], name=name)
        elif self.conf.auxiliary_classifier and self.conf.triplet_output:
            l_classification = Flatten()(l_before_output)
            # l_classification = Dense(2, activation='softmax')(l_classification)
            l_classification = Dense(2)(l_classification)
            l_classification = Lambda(lambda x: tf.nn.softmax(x))(l_classification)
            ...
            self.model = Model(d_input, [l_discrimination, l_classification, l_triplet], name=name)
        elif not self.conf.auxiliary_classifier and self.conf.triplet_output:
            l_triplet = Flatten()(l_before_output)
            l_triplet = Dense(int(f / 4), activation='linear')(l_triplet)
