...
class MiccaiLoader(Loader):

    def __init__(self):
        super(MiccaiLoader, self).__init__()
        self.num_anato_masks = 2
        self.num_patho_masks = 1
        self.num_volumes = 75
        self.input_shape = (224, 224, 1)
        self.data_folder = conf['miccai']
        self.log = logging.getLogger('miccai')
    ...
    def splits(self):
        ...
        splits = [
            {'validation': [1], # --> test on p11
             'test': [1],
             'training': [2,3,4,5]
             },
            {'validation': [2], # --> test on p13
             'test': [2],
             'training': [1, 3, 4, 5]
             },
            {'validation': [3],
            ...
            {'validation': [4], # --> test on p5
             'test': [4],
             'training': [2, 3, 1, 5]
             },
            {'validation': [5],  # --> test on p7
             'test': [5],
             'training': [2, 3, 4, 1]
             }
    ...
    def load_labelled_data(self, split, split_type, modality='LGE',
        ...
        if not os.path.exists(os.path.join(self.data_folder, npz_prefix + 'miccai_images.npz')):
            if modality == 'LGE':
                value_crop = False
            ...
            # save numpy arrays
            np.savez_compressed(os.path.join(self.data_folder, npz_prefix + 'miccai_images'), images)
            np.savez_compressed(os.path.join(self.data_folder, npz_prefix + 'miccai_masks_my'), masks_my)
            np.savez_compressed(os.path.join(self.data_folder, npz_prefix + 'miccai_masks_ven'), masks_ven)
            np.savez_compressed(os.path.join(self.data_folder, npz_prefix + 'miccai_masks_mi'), masks_mi)
            np.savez_compressed(os.path.join(self.data_folder, npz_prefix + 'miccai_patienet_index'), patient_index)
            np.savez_compressed(os.path.join(self.data_folder, npz_prefix + 'miccai_index'),index)
        # Load data from saved numpy arrays
        else:
            images        = np.load(os.path.join(self.data_folder, npz_prefix + 'miccai_images.npz'))['arr_0']
            masks_my      = np.load(os.path.join(self.data_folder, npz_prefix + 'miccai_masks_my.npz'))['arr_0']
            masks_ven     = np.load(os.path.join(self.data_folder, npz_prefix + 'miccai_masks_ven.npz'))['arr_0']
            masks_mi      = np.load(os.path.join(self.data_folder, npz_prefix + 'miccai_masks_mi.npz'))['arr_0']
            patient_index = np.load(os.path.join(self.data_folder, npz_prefix + 'miccai_index.npz'))['arr_0']
            index = np.load(os.path.join(self.data_folder, npz_prefix + 'miccai_index.npz'))['arr_0']
            slice = np.load(os.path.join(self.data_folder, npz_prefix + 'miccai_slice.npz'))['arr_0']
        ...
        self.log.debug('Loaded compressed miccai data of shape: ' + str(images.shape) + ' ' + str(index.shape))
    ...
    def load_unlabelled_data(self, split, split_type, modality='LGE', normalise=True, value_crop=True):
        ...
        :return:                a Data object
        """
        images, index = self.load_unlabelled_images('miccai', split, split_type, False, normalise, value_crop,modality=modality)
    ...
    def load_all_data(self, split, split_type, modality='MR', normalise=True, value_crop=True):
        ...
        :return:                a Data object
        """
        images, index = self.load_unlabelled_images('miccai', split, split_type, True, normalise, value_crop,modality=modality)
        masks = np.zeros(shape=(images.shape[:-1]) + (1,))
        scanner = np.array([modality] * index.shape[0])
        return Data(images, masks, '-1', index, scanner)
    ...
    def load_raw_labelled_data(self, normalise=True, value_crop=True):
        ...
        """
        self.log.debug('Loading miccai data from original location')
        ...
        for patient_i in self.volumes:
            ...
            mi_file_list = [file for file in os.listdir(patient_folder)
                            if (not file.startswith('.')) and (not file.find('MiSeg') == -1)]
            my_file_list = [file for file in os.listdir(patient_folder)
                            if (not file.startswith('.')) and (not file.find('MySeg') == -1)]
            ven_file_list = [file for file in os.listdir(patient_folder)
                            if (not file.startswith('.')) and (not file.find('VenSeg') == -1)]
            img_file_list.sort()
            mi_file_list.sort()
            my_file_list.sort()
            ven_file_list.sort()
            ...
            for v in range(volume_num):
                ...
                assert v_id_from_img==v_id_from_mi==v_id_from_my==v_id_from_ven, 'Mis-Alignment !'
    ...
    def load_raw_unlabelled_data(self, include_labelled=True, normalise=True, value_crop=True, modality='LGE'):
        ...
        """
        self.log.debug('Loading unlabelled miccai data from original location')
    ...
    def load_unlabelled_images(self, dataset, split, split_type, include_labelled, normalise, value_crop, modality):
        ...
        if os.path.exists(os.path.join(self.data_folder, npz_prefix + dataset + 'miccai_images.npz')):
            images = \
                np.load(os.path.join(self.data_folder,
                                     npz_prefix + dataset + 'miccai_images.npz'))['arr_0']
            index  = \
                np.load(os.path.join(self.data_folder,
                                     npz_prefix + dataset + 'miccai_index.npz'))['arr_0']
            patient_index = \
                np.load(os.path.join(self.data_folder,
                                     npz_prefix + dataset + 'miccai_patient_index.npz'))['arr_0']
        ...
        # Load from source
            ...
            np.savez_compressed(os.path.join(self.data_folder,
                                             npz_prefix + dataset + 'miccai_images'), images)
            np.savez_compressed(os.path.join(self.data_folder,
                                             npz_prefix + dataset + 'miccai_index'), index)
            np.savez_compressed(os.path.join(self.data_folder,
                                             npz_prefix + dataset + 'miccai_patient_index'), patient_index)
