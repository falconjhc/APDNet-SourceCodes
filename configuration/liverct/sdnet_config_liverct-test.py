import numpy as np
from configuration.liverct import unet_config_liverct
...
params = {
    ...
    'seed': 1,
    'folder': 'experiment_sdnet_liverct-test-Sup-l1-kl-rp-triplet',
    'data_len': 0,
    # 'epochs': 35,
    'batch_size': 8,
    'pool_size': 50,
    ...
    'input_shape': loader.LiverCtLoader().input_shape,
    'image_downsample': 1,
    'modality': 'LGE',
    ...
    'num_anatomy_masks': loader.LiverCtLoader().num_anato_masks,
    'out_anatomy_channels': loader.LiverCtLoader().num_anato_masks + 1,
    'num_pathology_masks': loader.LiverCtLoader().num_patho_masks,
    'out_pathology_channels': loader.LiverCtLoader().num_patho_masks + 1,
...
anatomy_encoder_params = copy.copy(unet_config_liverct.params)
anatomy_encoder_params['out_channels'] = params['num_mask_channels']
pathology_encoder_params = copy.copy(unet_config_liverct.params)
...
def get():
    ...
    params.update({'epochs':unet_config_liverct.params['epochs']})
    d_mask_anato_params['input_shape'] = (shp[:-1]) + (loader.LiverCtLoader().num_anato_masks,)
    d_mask_patho_params['input_shape'] = (shp[:-1]) + (loader.LiverCtLoader().num_patho_masks,)
