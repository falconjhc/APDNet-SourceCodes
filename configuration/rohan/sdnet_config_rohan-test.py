import numpy as np
from configuration.rohan import unet_config_rohan
...
params = {
    ...
    'seed': 1,
    'folder': 'experiment_sdnet_rohan-test-Sup-l1-kl-rp-triplet',
    'data_len': 0,
    # 'epochs': 35,
    'batch_size': 8,
    'pool_size': 50,
    ...
    'dataset_name': 'rohan',
    'test_dataset': 'rohan',
    'input_shape': loader.RohanLoader().input_shape,
    'image_downsample': 1,
    'modality': 'LGE',
    ...
    'num_anatomy_masks': loader.RohanLoader().num_anato_masks,
    'out_anatomy_channels': loader.RohanLoader().num_anato_masks + 1,
    'num_pathology_masks': loader.RohanLoader().num_patho_masks,
    'out_pathology_channels': loader.RohanLoader().num_patho_masks + 1,
...
anatomy_encoder_params = copy.copy(unet_config_rohan.params)
anatomy_encoder_params['out_channels'] = params['num_mask_channels']
pathology_encoder_params = copy.copy(unet_config_rohan.params)
...
def get():
    ...
    params.update({'epochs':unet_config_rohan.params['epochs']})
    d_mask_anato_params['input_shape'] = (shp[:-1]) + (loader.RohanLoader().num_anato_masks,)
    d_mask_patho_params['input_shape'] = (shp[:-1]) + (loader.RohanLoader().num_patho_masks,)
