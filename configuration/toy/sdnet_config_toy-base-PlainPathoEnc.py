import numpy as np
from configuration.toy  import unet_config_toy
...
params = {
    ...
    'seed': 1,
    'folder': 'experiment_sdnet_toy-Base-PlainPathoEnc',
    'data_len': 0,
    # 'epochs': 35,
    'batch_size': 8,
    'pool_size': 50,
    ...
    'dataset_name': 'toy',
    'test_dataset': 'toy',
    'input_shape': loader.ToyLoader().input_shape,
    'image_downsample': 1,
    'modality': 'LGE',
    ...
    'num_anatomy_masks': loader.ToyLoader().num_anato_masks,
    'out_anatomy_channels': loader.ToyLoader().num_anato_masks + 1,
    ...
    'pathology_encoder': 'plain' # 'unet' or 'plain'
}
...
anatomy_encoder_params = copy.copy(unet_config_toy.params)
anatomy_encoder_params['out_channels'] = params['num_mask_channels']
pathology_encoder_params = copy.copy(unet_config_toy.params)
...
def get():
    ...
    params.update({'epochs': unet_config_toy.params['epochs']})
    d_mask_anato_params['input_shape'] = (shp[:-1]) + (loader.ToyLoader().num_anato_masks,)
    d_mask_patho_params['input_shape'] = (shp[:-1]) + (loader.ToyLoader().num_patho_masks,)
