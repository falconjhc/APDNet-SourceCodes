import numpy as np
from configuration.cmr import unet_config_cmr
...
params = {
    ...
    'seed': 1,
    'folder': 'experiment_sdnet_cmr-Test-Sup-l1-kl-rp-triplet',
    'data_len': 0,
    # 'epochs': 35,
    'batch_size': 8,
    'pool_size': 50,
    ...
    'dataset_name': 'cmr',
    'test_dataset': 'cmr',
    'input_shape': loader.CmrLoader().input_shape,
    'image_downsample': 1,
    'modality': 'LGE',
    ...
    'num_anatomy_masks': loader.CmrLoader().num_anato_masks,
    'out_anatomy_channels': loader.CmrLoader().num_anato_masks + 1,
    'num_pathology_masks': loader.CmrLoader().num_patho_masks,
    'out_pathology_channels': loader.CmrLoader().num_patho_masks + 1,
...
anatomy_encoder_params = copy.copy(unet_config_cmr.params)
anatomy_encoder_params['out_channels'] = params['num_mask_channels']
pathology_encoder_params = copy.copy(unet_config_cmr.params)
...
def get():
    ...
    params.update({'epochs':unet_config_cmr.params['epochs']})
    d_mask_anato_params['input_shape'] = (shp[:-1]) + (loader.CmrLoader().num_anato_masks,)
    d_mask_patho_params['input_shape'] = (shp[:-1]) + (loader.CmrLoader().num_patho_masks,)
